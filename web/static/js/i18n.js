...
(function () {
  ...
  var DICT = {
    en: {
      ...
      "idx.noteStatic": "Everything runs in your browser — your file is never uploaded anywhere, and duplicates are removed automatically. The wheel &amp; tracklist images are generated in-browser (Python via WebAssembly), so the first run downloads the runtime once and may take a little while.",
      "py.runtime": "Loading the in-browser Python runtime… (first time only)",
      "py.packages": "Loading packages (numpy, pandas, matplotlib)…",
      "py.code": "Preparing…",
      "py.generating": "Generating images…",
      "py.error": "Couldn't generate the images. Open the browser console for details.",
      "res.done": "Done",
      ...
      "res.tlLabel": "Numbering",
      "tl.numDot": "1. 2. 3.",
      "tl.numParen": "(1) (2) (3)",
      "tl.numCircled": "① ② ③",
      "tl.plain": "No numbers",
      "tl.byTitle": "No numbers · Title A→Z",
      "tl.byArtist": "No numbers · Artist A→Z",
    ...
    ko: {
      ...
      "idx.noteStatic": "모든 처리가 브라우저 안에서 이뤄집니다 — 파일은 어디에도 업로드되지 않고, 중복은 자동 제거돼요. 휠·트랙리스트 이미지도 브라우저에서(WebAssembly 파이썬) 생성되며, 최초 1회는 런타임을 받느라 시간이 조금 걸립니다.",
      "py.runtime": "브라우저용 파이썬 런타임 불러오는 중… (최초 1회)",
      "py.packages": "패키지 불러오는 중 (numpy, pandas, matplotlib)…",
      "py.code": "준비 중…",
      "py.generating": "이미지 생성 중…",
      "py.error": "이미지를 생성하지 못했어요. 브라우저 콘솔을 확인해 주세요.",
      "res.done": "완성됐어요",
      "res.tracksUnit": "곡.",
      "res.from": "출처",
      "res.tracks": "트랙",
      ...
      "tl.numCircled": "① ② ③",
      "tl.plain": "번호 없음",
      "tl.byTitle": "번호 없음 · 제목순 (오름차순)",
      "tl.byArtist": "번호 없음 · 아티스트순 (오름차순)",
